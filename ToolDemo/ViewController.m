...
//    int const *a = 0;//指向常整型数的指针(指向的 整型数不可修改，但是指针可修改)
//    a = 1;
    
//    int * const a = 0;//指向整型数的常指针(指针指向的整型数可以修改，但是指针不可修改)
    
//    self.cache = [[NSCache alloc]init];
//    self.cache.countLimit = 10;//最大限制
//    self.cache.delegate = self;
   KLReflectionView *v = [[KLReflectionView alloc]initWithFrame:self.view.bounds];
...
-(void)semaphoreTest{
   ...
   NSBlockOperation * blockOperation = [NSBlockOperation blockOperationWithBlock:^{
       ...
       for (int i = 0; i<20; i++) {
           ...
           dispatch_async(dispatch_get_global_queue(0, 0), ^{
               NSLog(@"上传第===%d===任务. 当前线程是->%@.",i,[NSThread currentThread]);
               [NSThread sleepForTimeInterval:3];//沉睡2秒
               dispatch_semaphore_signal(semaphore);//+1
               NSLog(@"dispatch_semaphore_signal Begin.");
           });
